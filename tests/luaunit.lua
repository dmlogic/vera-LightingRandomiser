...
function assertGreaterThan(larger,smaller)

    if type(larger) == 'string' then larger = tonumber(larger) end
    if type(smaller) == 'string' then smaller = tonumber(smaller) end

    if larger <= smaller then
        errorMsg = "\n"..larger.." is not greater than "..smaller
        print(errorMsg)
        error(errorMsg,2)
    end
end

function assertLessThan(smaller,larger)

    if type(larger) == 'string' then larger = tonumber(larger) end
    if type(smaller) == 'string' then smaller = tonumber(smaller) end

    if smaller >= larger then
        errorMsg = "\n"..smaller.." is not less than "..larger
        print(errorMsg)
        error(errorMsg,2)
    end
end
